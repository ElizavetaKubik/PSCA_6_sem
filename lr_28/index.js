...
const bin_validator = (param) => {
  ...
  param.forEach((p) => {
    if (!isFinite(p)) throw new Error("Ожидается число");
  });
  return param;
};
...
server.on("div", bin_validator, (params, channel, resp) => {
  resp(null, params[0] / params[1]);
});
server.on("proc", bin_validator, (params, channel, resp) => {
  resp(null, (params[0] / params[1]) * 100);
});
server.on("sum", bin_validator, (params, channel, resp) => {
  resp(null, sumTo(params));
});
server.on("mul", bin_validator, (params, channel, resp) => {
  resp(null, mulTo(params));
});

server.listenHttp({ host: "127.0.0.1", port: 3000 }, () => {
  console.log("JSON-RPC Server");
});
